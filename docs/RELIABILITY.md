...
| Rental confirm/start/complete/cancel | Order/items/allocations, applicable inventory/history | Best effort after commit | RENTAL_ORDER_{status} in the same transaction |
...
Only rental creation supports the header. Identity remains (shopId, scope, key), backed
by the existing unique index. Hash calculation/comparison are unchanged. Completed claims
replay stored JSON including Decimal/Date strings and existing null semantics, without
repeating mutation or audit. Different payloads conflict before replay or stale recovery.
