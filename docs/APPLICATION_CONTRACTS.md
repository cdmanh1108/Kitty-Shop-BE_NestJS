...
There is one narrow assertion at the persisted replay boundary in `PrismaRentalRepository.claimIdempotency`: that scope is written by `createOrder` from `JSON.stringify(getWithTx(...))`. It does not validate externally edited/legacy malformed database JSON; adding a versioned replay schema is a separate idempotency task. No result is cast merely to hide a repository mapper mismatch, and no `as unknown as`/`any` was introduced.
