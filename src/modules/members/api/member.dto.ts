...
export class CreateMemberReqDto {
  ...
  @ApiProperty({ type: [String], example: ['STAFF'] }) @IsArray() @ArrayMinSize(1) @IsString({ each: true }) roleCodes!: string[];
}

export class UpdateMemberReqDto {
  @ApiPropertyOptional({ enum: ['ACTIVE', 'INACTIVE'] }) @IsIn(['ACTIVE', 'INACTIVE']) @IsOptional() status?: string;
  @ApiPropertyOptional({ type: [String] }) @IsArray() @ArrayMinSize(1) @IsString({ each: true }) @IsOptional() roleCodes?: string[];
}
