...
export class PrismaCustomerRepository implements CustomerRepository {
  ...
  async findById(shopId: string, id: string) {
    ...
    const [totalOrders, completedOrders, payments, recentOrders] = await this.prisma.$transaction([
      ...
      this.prisma.rentalOrder.findMany({
        ...
        select: { id: true, orderNumber: true, rentalStartAt: true, rentalEndAt: true, status: true, paymentStatus: true, grandTotal: true },
        orderBy: { createdAt: 'desc' },
        take: 10,
      }),
    ]);
  ...
  async update(shopId: string, id: string, input: Parameters<CustomerRepository['update']>[2]) {
    const existing = await this.prisma.customer.findFirst({ where: { id, shopId, archivedAt: null } });
    if (!existing) return null;
    return this.prisma.customer.update({ where: { id }, data: input });
  }

  addNote(input: { shopId: string; customerId: string; content: string; isPinned: boolean; createdBy: string }) {
  ...
  async addAddress(input: Parameters<CustomerRepository['addAddress']>[0]) {
    const customer = await this.prisma.customer.count({ where: { id: input.customerId, shopId: input.shopId, archivedAt: null } });
    if (!customer) return null;
  ...
  async updateAddress(input: Parameters<CustomerRepository['updateAddress']>[0]) {
    const address = await this.prisma.customerAddress.findFirst({
      where: { id: input.addressId, customerId: input.customerId, customer: { shopId: input.shopId, archivedAt: null } },
      select: { id: true },
    });
