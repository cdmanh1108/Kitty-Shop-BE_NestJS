...
export class PrismaCatalogRepository implements CatalogRepository {
  ...
  async createCategory(shopId: string, input: { code: string; name: string; parentId?: string }) {
    if (input.parentId) {
      const parent = await this.prisma.category.count({
  ...
  async upsertRentalRate(
    ...
    const existing = await this.prisma.rentalRate.findFirst({
      where: { shopId, variantId, durationDays: input.durationDays, isActive: true },
    });
    if (existing) {
      return this.prisma.rentalRate.update({
        where: { id: existing.id },
        data: { price: input.price },
      });
    }
    return this.prisma.rentalRate.create({
      data: {
        shopId,
  ...
  async removeProductMedia(shopId: string, productId: string, mediaId: string): Promise<boolean> {
    const deleted = await this.prisma.productMedia.deleteMany({
  ...
  async addInventoryItem(shopId: string, input: AddInventoryData) {
    ...
    if (!variant) return null;
    if (input.locationId) {
      const location = await this.prisma.shopLocation.count({
  ...
  async updateInventoryStatus(input: {
    shopId: string;
    id: string;
    status: string;
    condition?: string;
