  ...
  override async intercept(context: ExecutionContext, next: CallHandler) {
    try {
      ...
      if (error instanceof PayloadTooLargeException)
        throw new PayloadTooLargeException('Ảnh bằng chứng không được vượt quá 15 MB.');
      throw new BadRequestException(
        'Dữ liệu tải lên không hợp lệ. Chỉ được gửi một ảnh bằng chứng và các trường xác nhận.',
      );
    }
  }
}
